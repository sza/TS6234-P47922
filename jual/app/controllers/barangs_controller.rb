class BarangsController < ApplicationController
  ...
  def showads
  	@ad = Ad.find(params[:id])
    @ads = Ad.find(:all, :conditions=>["catid = ?", params[:id]])

    respond_to do |format|
      format.html # showads.html.erb
      format.xml  { render :xml => @ads }
    end
  end
  ...
  #def find
  	#@ads = Ad.find(:all, :conditions=>["title LIKE ? AND catid = ?","%" + params[:search_string] + "%", params[:category]])
 # end
end
